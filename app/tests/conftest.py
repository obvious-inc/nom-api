...
async def _create_random_wallet_user() -> User:
...
async def guest_user() -> User:
    return await _create_random_wallet_user()


@pytest.fixture
async def create_new_user():
    return _create_random_wallet_user
...
async def create_new_signer():
    async def _create_new_signer(user: User, broadcast: bool = True):
        ...
        if broadcast:
            await update_item(user, {"signers": [str_public_key]})
...
async def create_new_topic_channel():
    async def _create(user: Optional[User] = None):
        if not user:
            user = await _create_random_wallet_user()
